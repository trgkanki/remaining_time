...
"""
Remaining time
 - shows estimated time needed to complete the study
       : using exponential smoothing estimator
 - modified from plugin Progress bar
...
Copyright:  (c) Unknown author (nest0r/Ja-Dark?) 2017
            ...
            (c) Trgk 2017 <phu54321@naver.com>
...
limitToReviewSettings = False

# PROGRESS BAR APPEARANCE

qtxt = "black"  # Percentage color, if text visible.
qbg = "#ffffff"  # Background color of progress bar.
...
# If set to True, inverts and goes from right to left or top to bottom.
invertTF = False
...
# Stylesheet used only if blank. Else uses QPalette + theme style.
pbStyle = ""
...
def _dock(pb):
    ...
    existing_widgets = [widget for widget in mw.findChildren(
        QDockWidget) if mw.dockWidgetArea(widget) == dockArea]
...
def pb():
    ...
    global slopeEstimate
    slopeEstimate = ExponentialSmoother()
...
class ExponentialSmoother:
    ...
    def update(self, newPercent):
        timeSpent = min(time.time() - self.lastTime, 120)
        self.logs.append((timeSpent, newPercent))
    ...
    def getSlope(self):
        ...
        for i in range(1, len(self.logs)):
            r = 1.005 ** i
            totTime += r * (self.logs[i - 1][0])
            percentChange += r * (self.logs[i][1] - self.logs[i - 1][1])
        if totTime < 1:
            return 1
...
def _updatePB():
    ...
    if progressBar:
        ...
        if limitToReviewSettings and limitedBarLength != -1:
            ...
            # Can happen if "rev" cards increases during the study session
            if curr > barSize:
                barSize = curr
        ...
        if barSize == 0:
            ...
            newPercent = float(curr) / barSize
        slopeEstimate.update(newPercent)

        elapsedTime = slopeEstimate.elapsedTime
        remainingTime = (1 - newPercent) / slopeEstimate.getSlope()

        progressBar.hide()

        percent = 100 if barSize == 0 else 100.0 * curr / barSize
        progressBar.setFormat(
            "Elapsed %s,  Remaining %s, Total %s" %
            (
                t2s(elapsedTime),
                t2s(remainingTime),
